...
class XYZRelaFeature(object):
    ...
    def __init__(self, add_x=True, add_y=True, add_z=True):
        self._axis_rela = []
        axis_names = ["x_rela", "y_rela", "z_rela"]
        if add_x:
            self._axis_rela.append(0)
        if add_y:
            self._axis_rela.append(1)
        if add_z:
            self._axis_rela.append(2)

        self._axis_names_rela = [axis_names[idx_axis] for idx_axis in self._axis_rela]

    def __call__(self, data):
        assert data.pos is not None
        pos_centered = data.pos - data.pos.mean(axis=0)
        for axis_name, id_axis in zip(self._axis_names_rela, self._axis_rela):
            f = pos_centered[:, id_axis].clone()
            setattr(data, "pos_{}".format(axis_name), f)
        return data

    def __repr__(self):
        return "{}(axis={})".format(self.__class__.__name__, self._axis_names_rela)
